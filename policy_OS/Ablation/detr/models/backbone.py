...
class TactileBackbone(nn.Module):
    ...
    def __init__(self, name: str, ckpt: str, tac_names:list[str], train_backbone: bool, return_interm_layers: bool, position_embedding, tactile_type:Literal['feat', 'full']='feat'):
        ...
        train_config = os.environ.get('TRAIN_CONFIG', 'marked_rgb')
        if 'depth' in train_config:
            s = ['marked_rgb', 'depth']
        else:
            s = ['marked_rgb']
        s = ['marker', 'rgb', 'marked_rgb', 'pose', 'depth']
        # [
            # 'marked_rgb'
            # 'marked_rgb', 'depth'
            # 'marker', 'rgb', 'marked_rgb', 'pose', 'depth'
        # ]
        backbone = Tactile(backbone='resnet18', supervise=s, latent_dims=self.num_channels, norm_layer=FrozenBatchNorm2d)
