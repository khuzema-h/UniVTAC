    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, "r") as root:
            ...
            if len(available_cams) == 0:
                raise RuntimeError(f"No cameras found in {dataset_path}")

            ref_cam = available_cams[0]
            ref_img = root[f"/observations/images/{ref_cam}"][start_ts]
            ref_shape = ref_img.shape
            ref_dtype = ref_img.dtype

            for cam_name in self.camera_names:
                if cam_name in root["/observations/images"]:
                    image_dict[cam_name] = root[f"/observations/images/{cam_name}"][start_ts]
                else:
                    print(f"[WARN] Missing camera '{cam_name}' in {dataset_path}. Using zeros.")
                    image_dict[cam_name] = np.zeros(ref_shape, dtype=ref_dtype)
...
def worker_clear_fn():
    """Clean up HDF5 files for the current thread."""
...
class TacArenaDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        available_cams = list(root["/observations/images"].keys())
        if len(available_cams) == 0:
            raise RuntimeError(f"No cameras found in {hdf5_path}")
        ...
        for cam_name in self.camera_names:
            if cam_name in root["/observations/images"]:
                ...
                print(f"[WARN] Missing camera '{cam_name}' in {hdf5_path}. Using zeros.")
                image_dict[cam_name] = np.zeros(ref_shape, dtype=ref_dtype)
...
def load_data(dataset_dir, num_episodes, camera_names, tactile_names, batch_size_train, batch_size_val, chunk_size):
    ...
    print("camera_names requested:", camera_names)
