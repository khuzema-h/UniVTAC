...
def main(args):
    ...
    train_dataloader, val_dataloader, stats, _ = load_data(
        dataset_dir, num_episodes, camera_names, tactile_names,
    ...
    best_ckpt_info = train_bc(train_dataloader, val_dataloader, config, wandb_run)
...
def train_bc(train_dataloader, val_dataloader, config, wandb_run=None):
    ...
    while True:
        ...
        with torch.inference_mode():
            ...
            if wandb_run is not None:
                try:
                    val_to_log = {
                        f"val/{k}": float(v.item() if hasattr(v, "item") else v)
                        for k, v in epoch_summary.items()
                    }
    ...
    if wandb_run is not None and config.get("wandb_log_ckpts", False):
        try:
            ...
            artifact.add_file(ckpt_path)
            wandb_run.log_artifact(artifact, aliases=["last"])
        except Exception as exc:
